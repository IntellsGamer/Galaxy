...
|---------|-------------|----------------|
| **📄 Create File** | Create or overwrite files with AI-assisted content | `CREATE_FILE:filename.ext`<br>```language<br>content<br>``` |
| **✏️ Edit File** | Suggest complete edits to existing files | `EDIT_FILE:filename.ext`<br>```language<br>new content<br>``` |
| **🔍 Edit Region** | Replace specific sections in large files (recommended) | `EDIT_REGION:filename.ext`<br>`SEARCH:`<br>```language<br>exact snippet<br>```<br>`REPLACE:`<br>```language<br>new snippet<br>``` |
